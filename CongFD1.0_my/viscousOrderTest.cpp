 ...
 * @brief 粘性 MMS 阶数测定（整求解器驱动）：常黏组与幂律组（HIT 开发新增）
 *
...
// 单组：plExp、Tref 决定物性口径（plExp=0 即常黏）；返回该网格相对 L2 误差
static real runOne(real plExp,real Tref,int N)
{
    real L=2*PIT;
    real h=L/N;
    real dt=0.026*h*h;
    ...
    info->powerLawExp=plExp; info->Tref=Tref;     // HIT 开发：幂律组经此入链
...
{
    int Ns[3]={16,24,32};
    real par[2][2]={{0.0,100.0},{0.75,100.0}};
    const char* names[2]={"常黏组 (plExp=0)","幂律组 (plExp=0.75, Tref=100)"};
    int fail=0;
    for(int g=0;g<2;g++)
    {
        std::printf("---- %s ----\n",names[g]);
        ...
        {
            int N=Ns[tt];
            real dt=0.026*(2*PIT/N)*(2*PIT/N);
            long nstep=(long)std::llround(0.5/dt);
            errs[tt]=runOne(par[g][0],par[g][1],N);
            std::printf("MMS N=%d: dt=%.3e steps=%ld t=%.3f  相对L2=%.6e\n",N,dt,nstep,(real)(nstep*dt),errs[tt]);
        }
